...
- (void)buildView {
    ...
    [_changeButton.layer setCornerRadius:10.0];
    
    _phoneTextField.delegate = self;
    _phoneTextField.keyboardType = UIKeyboardTypeNumberPad;
    [_phoneTextField setText: _phone];
    ...
    [_changeButton addTarget:self action:@selector(clickSaveButton) forControlEvents:UIControlEventTouchUpInside];
...
- (void)clickSaveButton {
    ...
    __weak PassViewController *password = self;
    [HttpTool postWithPath:@"/user/verify" params:dict
                   success:^(id JSON) {
                       NSInteger status = [JSON[@"status"] integerValue];
                       if(0 == status) {
                           NSDictionary *data = JSON[@"data"];
                           [password savePassword:dict withVerify:data[@"verify"]];
                       }else {
                           [password giveErrorInfo:JSON[@"err"]];
                       }
                       ...
                       [alert addAction:action];
                       [password presentViewController:alert animated:YES completion:nil];
...
//修改并保存
- (void)savePassword:(NSMutableDictionary*)dict withVerify:(NSString *)verify{
   ...
   __weak PassViewController *password = self;
    [HttpTool postWithPath:@"/user/changePassword" params:dict
                   success:^(id JSON){
                       [_hud hide:YES];
                       NSInteger status = [JSON[@"status"] integerValue];
                       if(0 == status) {
                           [password.delegate changePassword:_sureTextField.text];
                           [password.navigationController popViewControllerAnimated:YES];
                       }else {
                           [password giveErrorInfo:JSON[@"err"]];
                       }
